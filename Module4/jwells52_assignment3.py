...
def write_results(sentences, tokens, stemmed_tokens, lemma_tokens):
    results_file = 'output/jwells52_assignment3/results.txt'
    ...
    with open(results_file, 'w') as f:
        for i in RESULT_INDICES:
                ...
                print(stemmed_tokens[i], file=f)
                print(file=f)

                print('LEMMATIZATION RESULTS', file=f)
                print(lemma_tokens[i], file=f)
                print('='*100, file=f)
                print(file=f)
